...
class PipelineUtils:
    @staticmethod
    def norm_text(s):
        """Normalize text keys for robust cross-source joins."""
        if s is None:
            return ""
        s = str(s)
    ...
    def coalesce(primary, secondary):
        ...
        if secondary is None:
            return primary
        primary_aligned, secondary_aligned = primary.align(secondary)
        return primary_aligned.where(primary_aligned.notna(), secondary_aligned)
...
class DataFrameBuilder:
    ...
    def build_fpl_df(data):
        ...
        fpl["player_key"] = fpl["player_name"].map(PipelineUtils.norm_text)
        fpl["team_key"] = fpl["name"].map(PipelineUtils.norm_text)

        rename_map = {
            c: f"FPL_{c}" for c in fpl.columns 
            if c not in ["player_key", "team_key"]
        }
    ...
    def build_understat_df(players):
        ...
        us["player_key"] = us["player_name"].map(PipelineUtils.norm_text)
        us["team_key_raw"] = us["team_title"].map(PipelineUtils.norm_text)
        return us
...
class MasterPipeline:
    ...
    def build_master(
        ...
        fpl_data = DataFetcher.fetch_fpl_bootstrap()
        fpl = DataFrameBuilder.build_fpl_df(fpl_data)

        us_players = DataFetcher.fetch_understat_league_players(season)
        us = DataFrameBuilder.build_understat_df(us_players)
        us, team_map_df = DataFrameBuilder.map_understat_teams_to_fpl(us, fpl)
        ...
        master["player_name"] = PipelineUtils.coalesce(raw.get("FPL_player_name"), raw.get("US_player_name"))
        master["team"] = PipelineUtils.coalesce(raw.get("FPL_name"), raw.get("US_team_title"))
        master["team_short"] = raw.get("FPL_short_name")
        master["position"] = PipelineUtils.coalesce(raw.get("FPL_singular_name_short"), raw.get("US_position"))
        ...
        master["minutes"] = PipelineUtils.coalesce(fpl_minutes, us_minutes) if "minutes" in prefer_fpl_for else PipelineUtils.coalesce(us_minutes, fpl_minutes)
        ...
        def pick_box(stat_name, fpl_col, us_col):
            ...
            if stat_name in prefer_fpl_for:
                return PipelineUtils.coalesce(f, u)
            return PipelineUtils.coalesce(u, f)
        ...
        # FPL-facing fields used directly in UI tables/cards.
        master["now_cost"] = pd.to_numeric(raw.get("FPL_now_cost"), errors="coerce") / 10
        master["total_points"] = raw.get("FPL_total_points")
        master["form"] = raw.get("FPL_form")
        ...
        # Derived columns
        master["xg_diff"] = master["goals"] - master["xG"]
        now_cost_safe = master["now_cost"].replace(0, pd.NA)
        master["value_pts"] = master["total_points"] / now_cost_safe

        for col in ["goals", "assists", "yellow_cards", "red_cards", "clean_sheets", "saves", "shots", "key_passes", "xG", "xA", "npg", "npxG"]:
            if col in master.columns:
                master[f"{col}_p90"] = PipelineUtils.per90(master[col], master["minutes"])
